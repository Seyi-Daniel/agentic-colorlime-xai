...
def test_catalog_has_metadata_for_every_registered_method():
    catalog = ToolCatalog()
    assert len(catalog.cards) == 3 * len(SEGMENTATION_FUNCTIONS)
    assert {card.segmentation_method for card in catalog.all_cards()} == set(SEGMENTATION_FUNCTIONS)
    for card in catalog.all_cards():
        assert set(card.to_dict()) == {"method", "function_name", "explainer", "segmentation"}
